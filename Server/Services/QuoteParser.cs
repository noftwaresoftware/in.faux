...
namespace Noftware.In.Faux.Server.Services
{
	...
	public class QuoteParser : IQuoteParser
	{
					...
					cleanLine.Contains("[") == true && cleanLine.Contains("]") == true)
					{
						...
						if (index2 > index1)
						{
							...
							if (quote.OriginalImage != null)
							{
								...
								ResizeImage(quote, (int)_settings.MaximumResizedImageDimension, out newImage, out newHeight, out newWidth);
		...
		private void ResizeImage(ParsedQuote quote, int maximumDimension, out byte[] newImageBytes, out int newImageHeight, out int newImageWidth)
		{
			...
			Image srcImage = (Bitmap)((new ImageConverter()).ConvertFrom(quote.OriginalImage));
